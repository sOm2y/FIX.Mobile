  ...
  selectTradies = (business, key) => {
    ...
    if (currentBusinessList.length > 0) {
      currentBusinessList.map((value, index) => {
        if (value.index === key) {
          currentBusinessList = [
            ...currentBusinessList.slice(0, index),
            ...currentBusinessList.slice(index + 1)
          ];
          isDuplicate = true;
        }
      });
      ...
      <Container>
        ...
        <View style={styles.container}>
          ...
          >
            {this.state.markers.map((marker, index) => (
              <TouchableOpacity
                onPress={() => this.selectTradies(marker, index)}
                style={styles.card}
                key={index}
              >
                {/* <Image
                ...
                resizeMode="cover"
              /> */}
                {this.state.selectedBusiness.length > 0 &&
                  this.state.selectedBusiness.map((value, key) => {
